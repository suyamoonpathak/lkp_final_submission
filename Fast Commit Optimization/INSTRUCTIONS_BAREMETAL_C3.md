...
# in REPO
cd ~/jbd2-project
sudo bash suyamoon/eval_baremetal_c3.sh
```

Runtime ~5 min. Auto-detects STOCK vs PATCHED by scanning your kernel
source tree for `bool touched_block` (absent = stock).

Results save to `suyamoon/eval_results_c3/baremetal/STOCK_<kernel>/`.

Commit and push right away:
```bash
# in REPO
cd ~/jbd2-project
git add suyamoon/eval_results_c3/baremetal/
...
patch -p1 < ~/jbd2-project/suyamoon/fc-inline-xattr.patch
...
Results save to `suyamoon/eval_results_c3/baremetal/PATCHED_<kernel>/`.
...
# in REPO
cd ~/jbd2-project
sudo bash suyamoon/c3_crash_test_a.sh   # 100 inline xattrs → crash → all must recover
sudo bash suyamoon/c3_crash_test_b.sh   # set 100, remove 50 → crash → exactly 50 remain
sudo bash suyamoon/c3_crash_test_c.sh   # 500-byte block xattr → crash → full value recovered
```
