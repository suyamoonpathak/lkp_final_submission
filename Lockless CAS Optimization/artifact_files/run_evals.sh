...
run_benchmark() {
    ...
    for run_id in 1 2 3; do
        ...
        for i in $(seq 1 $THREADS); do
            sudo mkdir -p $MNT/fsmark_$i
            D_CMD="$D_CMD -d $MNT/fsmark_$i"
        done
        # fs_mark natively binds exactly 1 thread per -d directory mapping!
        sudo fs_mark $D_CMD -n 1000 -s 4096 -S 1 > $RES_DIR/${PREFIX}_fsmark.txt
        sudo rm -rf $MNT/fsmark_*
        sync
        sudo sh -c 'echo 3 > /proc/sys/vm/drop_caches'
        sleep 10
        # 4. dbench
