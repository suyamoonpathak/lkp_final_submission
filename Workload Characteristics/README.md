...
```bash
cd "MTech Rocks/Workload Characteristics/benchmarks/wal_workload_no_fc"
...
```bash
cd "MTech Rocks/Workload Characteristics/benchmarks/combined_workload_no_fc"
...
- Fast commit is intentionally not enabled here. The companion fast
  commit work is in `Fast Commit Optimization/` (C3 inline xattr, C4 fallocate range)
  and `Lockless CAS Optimization/` (lockless wait-commit module).
